...
| `patches/openbor/0001-psc-pad-defaults.patch` | `control_pad_default_keys()` in `sdl/control.c`, called by `clearbuttons()` for players 1 and 2: on Linux the numbering of the virtual pad's **PlayStation Classic layout** (Triangle 0, Circle 1, Cross 2, Square 3, L2 4, R2 5, L1 6, R1 7, Select 8, Start 9, the D-pad on axes 0/1); on Windows (`_WIN32`) through `SDL_GameControllerGetBindForButton/Axis`, so any XInput pad gets the same layout (L2 is the left trigger, an axis). On Linux the codes are **fixed** (10 buttons, 2 axes - the layout pad.ini guarantees) and do not wait for an open pad: until `7533-4` they did, and on the console the game list's settings were applied before the pad counted, so the keyboard's keys stayed - the 2019 port likewise set fixed codes (601-614). Also: `open_joystick` names a pad by index with a NULL guard. |
...
| `patches/openbor/0006-game-list-hints-name-the-pad.patch` | **Linux only**: the game list's and the BGM player's hints name the buttons the actions are bound to (`X: Start Game`, `O: BGM Player`, `R1: Quit Game`; `P1 ` dropped to fit) - upstream hard-codes the keyboard's `A:`/`S:`/`D:`/`F:` since 2021 (Kratus), which is why 0005's names never showed there; and no log viewer (the list's L1 does nothing, the logs are not read) - both as the 2019 port had it. From `7533-4` |
